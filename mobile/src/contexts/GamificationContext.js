...
export function GamificationProvider({ children }) {
  ...
  const fetchXP = async () => {
    const r = await fetch(`${API}/xp`, { headers: await getHeaders() });
  ...
  const fetchCollectibles = async () => {
    const headers = await getHeaders();
    const [all, mine] = await Promise.all([
      fetch(`${API}/collectibles`, { headers }),
      fetch(`${API}/my-collectibles`, { headers }),
    ]);
  ...
  const fetchLevels = async () => {
    const r = await fetch(`${API}/levels`, { headers: await getHeaders() });
    if (r.ok) setLevels((await r.json()).data || []);
  };

  const fetchLeaderboard = async () => {
    const r = await fetch(`${API}/leaderboard`, { headers: await getHeaders() });
  ...
  const runDailyLogin = useCallback(
    async () => {
      const today = new Date().toDateString();
      if (lastLoginDate === today) return;
      try {
        const r = await fetch(`${API}/daily-login`, { method: 'POST', headers: await getHeaders() });
        if (!r.ok) return;
        const d = await r.json();
  ...
  const awardCollectible = useCallback(
    async (collectibleId) => {
      try {
  ...
  const getCurrentLevelData = useCallback(
    () => levels.find((l) => l.level === (user?.level || 1)) || levels[0],
    [levels, user?.level]
  );
  const getNextLevelData = useCallback(
    () => levels.find((l) => l.level === (user?.level || 1) + 1),
    [levels, user?.level]
  );
