...
router.put('/accessibility', authMiddleware, async (req, res) => {
  try {
    const { fontSize, highContrast, reduceMotion, voice, dateFormat, timezone } = req.body;
    const accessibility = { fontSize, highContrast, reduceMotion, voice, dateFormat, timezone };
...
router.delete('/account', authMiddleware, async (req, res) => {
  try {
    ...
    // Clean up NeDB content data
    await db.sessions.remove({ userId: String(req.user.id) }, { multi: true });
    await db.saved.remove({ userId: String(req.user.id) }, { multi: true });
    await db.history.remove({ userId: String(req.user.id) }, { multi: true });
    await db.contributions.remove({ userId: String(req.user.id) }, { multi: true });
    await db.notifications.remove({ userId: String(req.user.id) }, { multi: true });
...
router.post('/deactivate', authMiddleware, async (req, res) => {
  try {
    ...
    await db.sessions.remove({ userId: String(req.user.id) }, { multi: true });
    res.json({ message: 'Account deactivated. You can reactivate by contacting support.' });
  } catch (err) {
...
router.get('/sessions', authMiddleware, async (req, res) => {
  try {
    const sessions = await db.sessions.find({ userId: String(req.user.id) });
    res.json({ sessions: sessions.map(s => ({ id: s._id, createdAt: s.createdAt, expiresAt: s.expiresAt })) });
  } catch (err) {
...
router.delete('/sessions/:id', authMiddleware, async (req, res) => {
  try {
    await db.sessions.remove({ _id: req.params.id, userId: String(req.user.id) }, {});
...
router.get('/export-data', authMiddleware, async (req, res) => {
  try {
    const { password, otp, otp_expires, ...safeUser } = req.user;
    const saved = await db.saved.find({ userId: String(req.user.id) });
    const history = await db.history.find({ userId: String(req.user.id) });
    const contributions = await db.contributions.find({ userId: String(req.user.id) });
