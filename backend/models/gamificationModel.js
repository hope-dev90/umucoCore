...
async function checkAndAwardBadges(
  ...
  for (const badge of result.rows) {
    let qualifies = false;
    if (badge.trigger_type === "xp_milestone" && userXP >= badge.trigger_value)
      qualifies = true;
    if (badge.trigger_type === "level" && userLevel >= badge.trigger_value)
      qualifies = true;
    if (badge.trigger_type === "streak" && userStreak >= badge.trigger_value)
      qualifies = true;
...
export const awardBadge = async (userId, badgeId) => {
  ...
  if (result.rows.length > 0) {
    const badgeResult = await pool.query(`SELECT * FROM badges WHERE id = $1`, [
      badgeId,
    ]);
    if (badgeResult.rows[0].xp_reward > 0) {
      await awardXP(
        userId,
        badgeResult.rows[0].xp_reward,
        `Badge: ${badgeResult.rows[0].name}`,
      );
    }
  }
  return result.rows[0];
...
export const getAllCollectibles = async () => {
  const result = await pool.query(
    `SELECT * FROM collectibles ORDER BY rarity ASC`,
  );
  return result.rows;
};
...
// Get all item_ids for a user and activity type (for read proverbs, riddles, etc.)
export const getUserActivityItems = async (userId, activityType) => {
  const result = await pool.query(
    `SELECT item_id FROM user_activity_log WHERE user_id = $1 AND activity_type = $2`,
    [userId, activityType],
  );
  ...
  switch (activityType) {
    case "riddle":
      ...
      if (metadata.revealed)
        return {
          xp: metadata.correct === false ? 5 : 3,
          reason: "Riddle: revealed answer",
        };
...
export const trackActivity = async (
  ...
  try {
    ...
    const newBadges = await checkActivityBadges(
      client,
      userId,
      activityType,
      newCount,
    ...
    return {
      ...
      newBadges,
      activityCounts: {
        [ACTIVITY_TRIGGER_MAP[activityType] || activityType]: newCount,
      },
    };
  } catch (error) {
    await client.query("ROLLBACK");
